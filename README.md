...
```bash
python3 backup-user.py
```

### Requirements

This program relies on interactive terminal prompts rather than command-line arguments and enforces strict system requirements upon initialization:

* **Operating System**: Cross-platform compatible (Windows, Linux, macOS).

* **Privileges**: Must be executed as a standard user. The script will fatally exit if run as `root` (Linux/macOS) or Administrator (Windows) to prevent system-level modifications and preserve intended file permissions.

* **Dependencies**: The `7z` executable must be installed. The script dynamically searches for the binary in your system's PATH as well as standard OS installation directories (e.g., `C:\Program Files\7-Zip\`, `/usr/bin/`).
